...
# key local paths
v_isbncheck_path = os.path.join(sys.path[0], '..', 'xml_docx_stylechecks', 'validator_isbncheck.py')
transform_testfiles_dir = os.path.join(sys.path[0], 'files_for_test', 'full_transform', 'test_docx_files')
validfiles_basedir = os.path.join(sys.path[0], 'files_for_test', 'full_transform', 'vi_validated_output')
tmpdir_base = os.path.join(sys.path[0], 'files_for_test', 'tmp', 'vi_diff')
diff_outputdir = os.path.join(tmpdir_base, 'vi_diff_outputs_{}'.format(time.strftime("%y%m%d%H%M%S")))
